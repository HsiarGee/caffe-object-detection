/*
 * ===  ABOUT  =========================================================================
 *  Description: Detect and classify by using Faster R-CNN model, save result in jpeg
 *               images.
 *       Author: Yihang Lou, modified by Gee Hsiar
 *         Date: 8/31/2018
 * =====================================================================================
 */
...
/*string CLASSES[class_num] = {"background",  //always index 0                           
	"normalseam", "overseam", "underseam"};
*/
 ...
 * =====================================================================================
 */
//perform detection operation: input image max size 1000*600
cv::Mat Detector::Detect(const string& im_name)
{
    ...
    case Caffe::GPU:
        ...
        break;
    default:
       std::cout << "Unknow Caffe mode" << endl;
    }
	...
	for (int i = 1; i < class_num; i ++)
	{
        ...
        //for visualize only
		vis_detections(cv_img, keep, num_out, sorted_pred_cls, i, CONF_THRESH);
	}

    //cv::imwrite(out_name, cv_img);
	return cv_img;
 ...
 * =====================================================================================
 */
void Detector::vis_detections(cv::Mat image, int* keep, int num_out, float* sorted_pred_cls, int lable, float CONF_THRESH)
{
	int i = 0;
	while(sorted_pred_cls[keep[i]*5+4]>CONF_THRESH && i < num_out)
	{
		...
		//cout << "Confidence:" << sorted_pred_cls[keep[i]*5+4] << endl;
		//cout << "Lable:" << lable << endl;
		cv::rectangle(image,cv::Point(sorted_pred_cls[keep[i]*5+0],sorted_pred_cls[keep[i]*5+1]),cv::Point(sorted_pred_cls[keep[i]*5+2], sorted_pred_cls[keep[i]*5+3]),cv::Scalar(0,0,255),2);
		
		string text = CLASSES[lable] + ":" + to_string(sorted_pred_cls[keep[i]*5+4]);
		int fontFace = cv::FONT_HERSHEY_SIMPLEX;
		double fontScale = 1.0;
		int thickness = 2;
		int baseline = 0;
		cv::Size textSize = cv::getTextSize(text, fontFace, fontScale, thickness, &baseline);
		cv::Point textOrg(sorted_pred_cls[keep[i]*5+0], sorted_pred_cls[keep[i]*5+1] - textSize.height);
